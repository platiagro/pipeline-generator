...
from pipelines.jupyter import JUPYTER_ENDPOINT, COOKIES, HEADERS
...
class TestOperators(TestCase):
    ...
    def setUp(self):
        ...
        session.put(
            url=f"{JUPYTER_ENDPOINT}/api/contents/experiments/{TRAINING_ID}/operators/{OPERATOR_ID_2}",
            data=dumps({"type": "directory", "content": None}),
        )

        session.put(
            url=f"{JUPYTER_ENDPOINT}/api/contents/experiments/{TRAINING_ID}/operators/{OPERATOR_ID_2}/Experiment.ipynb",
            data=dumps({"type": "notebook", "content": loads(SAMPLE_COMPLETED_NOTEBOOK)}),
        )

    def test_get_logs(self):
        with app.test_client() as c:
            rv = c.get(f"/trainings/{TRAINING_ID}/operators/{OPERATOR_ID}/logs")
            result = rv.get_json()
            ...
            rv = c.get(f"/trainings/{TRAINING_ID}/operators/{WRONG_OPERATOR_ID}/logs")
            result = rv.get_json()
            ...
            # NOTE: enable this test when there is an instance of Kubernetes installed in Github Actions,
            # since if there are no logs in the Jupyter API, it will be searched in the operator's container
            # ------------------------------------------------------------------------------------------------
            # rv = c.get(f"/trainings/{TRAINING_ID}/operators/{OPERATOR_ID_2}/logs")
            # result = rv.get_json()
